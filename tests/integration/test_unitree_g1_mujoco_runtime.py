...
def test_joint_motion_feedback_and_reset(robot):
    ...
    sim = robot._native
    target = np.zeros(sim.ik.size)
    target[[0, sim.ik.size // 2]] = -0.3
    target[[3, sim.ik.size // 2 + 3]] = 0.5
...
def test_gravity_matches_mujoco_and_reduces_sag(robot):
    ...
    q[[0, sim.ik.size // 2]] = -0.7
    q[[3, sim.ik.size // 2 + 3]] = 0.7
    body = sim.ik.scatter_arm(q)
    sim.reset(body)
    np.testing.assert_allclose(sim.ik.gravity(q), sim.data.qfrc_bias[sim.vadr], atol=1e-7)
    errors = []
...
def test_robot_camera_changes_with_motion(robot):
    ...
    sim = robot._native
    q = np.zeros(sim.ik.size)
    q[[0, sim.ik.size // 2]] = -0.8
    q[[3, sim.ik.size // 2 + 3]] = 1
