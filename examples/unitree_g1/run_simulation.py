...
"""Run local G1 arm dynamics with joint-space targets; no Cartesian solver."""
...
def main():
    ...
    try:
        ...
        def key(event):
            ...
            if event.keysym in joints:
                index, delta = joints[event.keysym]
                q[[index, sim.size // 2 + index]] += delta
                np.clip(q, sim.lower, sim.upper, out=q)
        ...
        print("Arrows: shoulder pitch/roll; W/S: elbow angles. Close window to stop.")
        tick()
        root.mainloop()
    finally:
        robot.disconnect()
